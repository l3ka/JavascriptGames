import {squares, result, displayCurrentPlayer} from './Helpers.js'
        ...
        item.addEventListener('click', function onClick() {
            if (squares[i + 7].classList.contains('taken')) {
                if (currentPlayer === 1) {
                    ...
                    displayCurrentPlayer.innerHTML = (currentPlayer = 2) + ' (blue)';
                }
                    ...
                    displayCurrentPlayer.innerHTML = (currentPlayer = 1) + ' (red)';
                }            
            }
            else {
    ...
    for (let i = 0; i < winningArrays.length; ++i) {
        ...
        if (square1.classList.contains('player-one') && square2.classList.contains('player-one') && 
            square3.classList.contains('player-one') && square4.classList.contains('player-one')) {
                result.innerHTML = "Congratulation! Player One Wins";
                reset();
            }
            ...
            result.innerHTML = "Congratulation! Player Two Wins";
            reset();
        }
    }
}
...
export {initSquare as startGame};